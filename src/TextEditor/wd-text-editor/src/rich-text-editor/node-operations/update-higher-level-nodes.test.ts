...
describe('update-higher-level-nodes', () => {
    ...
    it('returns null for an invalid index', () => {
        const result = updateHigherLevelNodes(-1, [], [], mockHistoryManager, null, 0, 0);
      ...
      it('inserts nodes at the beginning when insertAt is not specified', () => {
        ...
        const result = updateHigherLevelNodes(0, higherLevelChildren, higherLevelChildren[0].Children, mockHistoryManager, newNodes, 0, 0);
      ...
      it('inserts nodes at the end when insertAt is "end"', () => {
        const higherLevelChildren = mockTableCellData.map(c => deepCopyAstNode(c));
        const newNodes = [ createNewAstNode('Text', 0, 0, 'Hello World') ];
        const result = updateHigherLevelNodes(0, higherLevelChildren, higherLevelChildren[0].Children, mockHistoryManager, newNodes, 0, higherLevelChildren[0].Children.length - 1, 'end');
        expect(result).not.toBe(null);
